...
void draw()
{
	...
	mat4 model2 = translate(mat4(1.0f), vec3(1.0f, 0.0f, -2.0f));
	mat4 model4 = glm::scale(mat4(1.0f), vec3(3.0, 3.0, 3.0));
...
void key_callback(GLFWwindow * window, int key, int scancode, int action, int mods) {
	...
	// FPS CAMERA CONTROLS
		...
		// Forward (UP arrow) 
		if (key == GLFW_KEY_UP && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
			cameraPos = cameraPos + F * cameraSpeed;
			cameraCenter = cameraCenter + F * cameraSpeed;
		}
		...
		// Left (LEFT arrow) 
		if (key == GLFW_KEY_LEFT && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
			cameraPos = cameraPos - S * cameraSpeed;
			cameraCenter = cameraCenter - S * cameraSpeed;
		}
